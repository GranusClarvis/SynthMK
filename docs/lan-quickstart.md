...
- Screenshot server: <http://localhost:9180/> (served by the runner node)
...
Run discovery on `intranet-demo` → `Synthetic Account Overview` appears **under
its own host**. One runner, results attributed per target site.
...
Each flow runs on its own interval; slow checks never block fast ones.
