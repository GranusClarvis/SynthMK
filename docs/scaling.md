...
| Startup stagger: first runs spread out, no t=0 stampede | automatic | n/a |
| Overlap suppression: a still-running flow is never stacked | automatic (counted in perfdata) | n/a |
| Hard run timeout: hung browser killed, slot freed, UNKNOWN reported | `SYNTHMK_RUN_TIMEOUT_S` | 180 |
| Warmup lines: instant discoverable result per flow at start | automatic | n/a |
| Self-monitoring: `SynthMK Scheduler` service WARNs when flows go overdue | automatic, every 30s | n/a |

**The contract: if the node is oversubscribed, the `SynthMK Scheduler` service
goes WARN on the node** (with names of overdue flows), so monitored applications
